                           ...
                           std::shared_ptr<sdbusplus::asio::connection>& conn)
{
    ...
    asyncCallTimer->expires_after(std::chrono::seconds(timeout));
    asyncCallTimer->async_wait(
        [&conn, &server](const boost::system::error_code& ec) {
            ...
            else
            {
                ...
                pfr::pfrConfigObject =
                    std::make_unique<pfr::PfrConfig>(server, conn);
