...
static const boost::container::flat_map<uint8_t,
                                        std::pair<std::string, std::string>>
    panicReasonMap = {
        {0x01, {"FirmwareResiliencyNotify", "BIOS update intent"}},
        {0x02, {"FirmwareResiliencyNotify", "BMC update intent"}},
...
static void logLastPanicEvent()
{
    ...
    std::string msgId = "OpenBMC.0.1." + it->second.first;
    sd_journal_send(
        "MESSAGE=%s", "Platform firmware resiliency event occurred.",
        "PRIORITY=%i", LOG_WARNING, "REDFISH_MESSAGE_ID=%s", msgId.c_str(),
