...
#Read PFR property
support=$(busctl get-property xyz.openbmc_project.EntityManager $board/PFR xyz.openbmc_project.Configuration.PFR Supported | cut -b 4- | sed 's/.$//')

echo "PFR Supported=$support"

if [[ "$support" == 'true' ]] && pfr_active_mode
then
